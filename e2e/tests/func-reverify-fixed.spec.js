...
test('SEATS: conversion preview + confirm adds a history row', async ({ page }) => {
  await boot(page);
  await gotoNav(page, 'Merit', 'Seat Allocation');
  await page.locator('button:has-text("Seat Conversion")').click();
  await page.waitForTimeout(400);

  // Defaults source=bscai-gen, target=mba-gen are valid (different) — no need to reselect.
  await page.locator('input[type="number"]').first().fill('3');
  await page.locator('button:has-text("Preview")').click();
  await page.waitForTimeout(400);
  await page.locator('button:has-text("Confirm")').click();
  await page.waitForTimeout(500);
  // A conversion history row with an audit reference must now exist.
  await expect(page.locator('text=/AUD-/').first()).toBeVisible();
});

// BUG-SEAT-01: the Matrix tab's direct "Sanctioned" edit used to skip both guardrails Convert
// already enforces — it could drop sanctioned below admitted, and wrote no audit entry at all.
test('SEATS: Matrix direct edit enforces the admitted floor and writes an audit entry', async ({ page }) => {
  await boot(page);
  await gotoNav(page, 'Merit', 'Seat Allocation');
  await page.waitForTimeout(400);

  // BSc AI – General: sanctioned 60, admitted 45 (seeded default).
  const row = page.locator('tr', { hasText: 'BSc AI' }).filter({ hasText: 'General' });
  await row.locator('button:has-text("Edit")').click();
  await page.waitForTimeout(200);

  // Below the admitted floor (45) must be rejected with no change.
  await row.locator('input[type="number"]').fill('10');
  await row.locator('button:has-text("Save")').click();
  await page.waitForTimeout(300);
  await expect(page.locator('.toast-pop')).toContainText('admitted');
  await expect(row.locator('input[type="number"]')).toBeVisible(); // still in edit mode, unsaved

  // A valid increase must save AND leave an audit trail, same as a Convert — the trail lives on
  // the Seat Conversion tab's history table, so switch there to check it.
  await page.locator('button:has-text("Seat Conversion")').click();
  await page.waitForTimeout(300);
  const audBefore = await page.locator('text=/AUD-/').count();
  await page.locator('button:has-text("Seat Matrix")').click();
  await page.waitForTimeout(300);

  await row.locator('input[type="number"]').fill('65');
  await row.locator('button:has-text("Save")').click();
  await page.waitForTimeout(400);
  ...
  await page.locator('button:has-text("Seat Conversion")').click();
  await page.waitForTimeout(300);
  const audAfter = await page.locator('text=/AUD-/').count();
  expect(audAfter).toBeGreaterThan(audBefore);
  await expect(page.locator('text=direct edit')).toBeVisible();
});
